...
class ProductSelect extends Component {
    ...
    handleAddToReceipt = () => {
        ...
        localStorage.setItem('EmulsionData', JSON.stringify({ todoList, total }));
    ...
    render() {
        ...
        const uniqueBases = [...new Set(baseOptions.map(item => item.base))];

        return (
            <div className="container">
                <h1 className="heading">Emulsions</h1>
                ...
                {baseOptions.length > 0 && (
                    <>
                        <select className="base-select" onChange={this.handleBaseSelect} value={selectedBase}>
                            ...
                            {uniqueBases.map((baseItem, index) => (
                                <option key={index} value={baseItem}>{baseItem}</option>
                            ))}
                        </select>
